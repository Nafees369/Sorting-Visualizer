...
const quickSort = (array, position, arraySteps, colorSteps) => {
  ...
  if (array.length < 2) {
    ...
    // Set the color of the single element to indicate it's in its final sorted position
  ...
  // Pick the median of three numbers as the pivot and move it to the back
  ...
  // Swap small value from the right with a big value from the left
  while (A < B) {
    while (array[A] < pivot) {
      insertStep(array, position, arraySteps);
      let colorKey = colorSteps[colorSteps.length - 1].slice();
  ...
  // Swap the big value with the pivot
...
// Function to pick the pivot as the median of three numbers
